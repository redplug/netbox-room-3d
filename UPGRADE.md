...
이번 버전은 DB 모델·마이그레이션 변경이 없으며 기존 JSON 데이터를 그대로 읽습니다. 선택한 격자 시작 모서리는 `grid_origin`으로 저장되며, 기존 데이터에 값이 없으면 좌상으로 처리합니다. 기존 서버실 크기,
랙 중심 좌표·회전·잠금, 장애물, 장비 색상·이미지 지정 및 revision을 그대로 사용합니다.
